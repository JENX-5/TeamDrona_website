    ...
    <div id="intro-overlay" onClick={handleClick} style={{ pointerEvents: zipOpen ? 'none' : 'auto' }}>
      ...
      <div className={`intro-center ${fired ? 'intro-fade' : ''}`}>
        <svg viewBox="0 0 200 340" width="200" height="340" style={{ overflow: 'visible' }}>
          {/* Bow — correct orientation: curves right (arrow on left) */}
          <path
          ...
          />
          <line
            className={phase >= 2 ? (pulled ? ' str-pull' : fired ? ' str-snap' : '') : ''}
            x1={cx} y1={topY} x2={pulled ? cx - 30 : cx} y2={midY}
            stroke="#d4c4a0" strokeWidth="1.5" strokeLinecap="round"
            opacity={phase >= 2 ? 1 : 0}
          />
          <line
            className={phase >= 2 ? (pulled ? ' str-pull' : fired ? ' str-snap' : '') : ''}
            x1={pulled ? cx - 30 : cx} y1={midY} x2={cx} y2={botY}
            stroke="#d4c4a0" strokeWidth="1.5" strokeLinecap="round"
            opacity={phase >= 2 ? 1 : 0}
          />
          <g className={`arrow-g${pulled ? ' pulled' : ''}${fired ? ' fired' : ''}`}>
            ...
            <polygon
              ...
              fill="var(--saffron)" opacity={phase >= 2 ? 1 : 0}
            />
